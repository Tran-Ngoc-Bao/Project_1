...
void loop()
{
    ...
    if (cnt % 6 == 5)
    {
        ...
        client.publish("NamDinh/data", buffer);
        if (d > 2.5 && d <= 2.7)
        {
            client.publish("NamDinh/warning/1", buffer);
        }
        else if (d > 2.7 && d <= 3)
        {
            client.publish("NamDinh/warning/2", buffer);
        }
        else if (d > 3)
        {
            client.publish("NamDinh/warning/3", buffer);
        }
        sum = 0;
        ++cnt;
    }
